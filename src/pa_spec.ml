...
module Make (Syntax : Sig.Camlp4Syntax) = struct
  ...
  let matching_expecation _loc case res =
    ...
    <:expr<
  match $res$ with
  | $case$ -> Simplespec.Spec.add_successful_expectation example 
...
>>
  ...
  EXTEND Gram
    GLOBAL: expr match_case match_case0;
    expr: LEVEL "simple" [
      ...
      | res = SELF ; "should" ; OPT "be" ; "(" ; "fun" ; args = LIST1 ipatt; "->" ;
        e = SELF ; ")" ; exp = OPT expr LEVEL "top" -> function_expectation _loc args e res exp;
      (* 強制的に失敗させる *)
